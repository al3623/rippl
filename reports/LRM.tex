...
\title{Rippl Programming Language\\Reference Manual}
\author{Da Hua Chen (Test Monkey), Hollis Lehv (Gallbladder),\\Amanda Liu (Language Yoda), Hans Montero (Prime Minister)
...
Given the functional nature of Rippl, there are no assignment statements. Instead, Rippl consists mainly of nested expressions strung together
...
\texttt{if bool then expr else expr}. Although the \texttt{if} and \texttt{then} clauses are mandatory, the \texttt{else} clause is not.
\begin{lstlisting}[language=rippl]
let num = 5 in
