...
let rec get_closure_vars exp scope nested= match exp with
	| Let(Assign(nl, Lambda(_,_)), e1) -> find_lambdas nested exp; get_closure_vars e1 (StringSet.add nl scope) nested
	...
	| Lambda(e2, e3) -> find_lambdas nested exp; StringSet.empty
...
and find_lambdas nested = function
	| Let(Assign(ln, Lambda(Var(p), e8)), e9) ->
		...
		print_endline ("lambda name:" ^ ln ^ ";param: " ^ p ^ ";inner_vars[ ");
		StringSet.iter (print_endline) vs1;
		print_endline "]";
		if not nested then find_lambdas nested e9 else ();
    | Let(_ ,e1) -> find_lambdas nested e1;
    | App(e2, e3) -> find_lambdas nested e2; find_lambdas nested e3;
    | Ite(e4, e5, e6) -> find_lambdas nested e4; find_lambdas nested e5; find_lambdas nested e6;
